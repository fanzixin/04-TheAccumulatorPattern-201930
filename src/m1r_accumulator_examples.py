...
#          what expression would you use in the _____ below?
#
#            for k in range(__s-r-14__):      # (s-10) - (r+4) = s - r -14
#
...
#
#              for k in range(r - s):      <== (r-s) should be (s-r+1)
#                  count = 0               <== (count=0) should be put before the for loop
#                  if math.cos(r) > 0:     <== (r) should be (k+r)
#                      count = 1           <== (count=1) should be (count=count+1)
#                  return count            <== (return count) should be out of the for loop
#
#     f. The code in the "graphics accumulation" example below includes:
#            for _ in range(n):
#        What does the   _   (underscore) mean?     <== a counter index; it won't be used inside the for loop.
#
...
#              x = starting_point.x
#              for k in range(n):
#                  center = rg.Point(x + (k * diameter), y)        <== This one is right.
#                  circle = rg.Circle(point, radius)
#                  circle.attach_to(window)
#
#              x = starting_point.x
...
#                  circle = rg.Circle(point, radius)
#                  circle.attach_to(window)
#                  x = x + (2 * radius)                            <== This one is wrong because x shouldn't be changed in this method.
#
###############################################################################
